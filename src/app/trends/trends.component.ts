...
export class TrendsComponent {
  ...
  refreshChartDataPerGame() {
    ...
    this.dataForGraph.sort(
      (a, b) => new Date(a.GameDate).getTime() - new Date(b.GameDate).getTime()
    );
    const groupedByPlayerName =
      _.groupBy(this.dataForGraph, (data: any) => data.PlayerName) || [];
    const dataSets = _.map(groupedByPlayerName, (grp, i) => {
      return {
        label: grp[0].PlayerName,
        data: grp.map(
          (gr: any) =>
            gr.PlayerStats[this.statCategory] &&
            gr.PlayerStats[this.statCategory][this.statToGraph]
        ),
        fill: false,
        yAxisID: 'y',
        tension: 0.4,
    ...
    const labels = _.map(
      _.uniqBy(this.dataForGraph, (dat) => dat.GameDate),
      (item) => this.datePipe.transform(item.GameDate)
    );
    this.chartData = {
      labels: labels,
      datasets: dataSets,
  ...
  refreshChartDataPerMonth() {
    ...
    _.forEach(groupedByPlayerName, (playerGames: any[], i) => {
      const groupedByPlayerNameAndMonth =
        _.groupBy(playerGames, (data: any) =>
          new Date(data.GameDate).getMonth()
        ) || [];
      _.forEach(groupedByPlayerNameAndMonth, (monthlyGames: any) => {
        ...
        _.forEach(monthlyGames, (monthlyGame) => {
          let indivGameStat: number =
            monthlyGame.PlayerStats[this.statCategory] &&
            monthlyGame.PlayerStats[this.statCategory][this.statToGraph];
        ...
        rawDataSets.push({
          ...
          Month: month,
          Value: !this.totals
            ? avgMonthStat / monthlyGames.length
            : avgMonthStat,
        });
      });
    });

    const rawGroupedByPlayerName =
      _.groupBy(rawDataSets, (data: any) => data.Name) || [];
    ...
    const labels = _.map(
      _.uniqBy(rawDataSets, (dat) => dat.Month),
      (item) => this.months[item.Month]
    );
    this.chartData = {
      labels: labels,
      datasets: dataSets,
  ...
  refreshChartDataTotals() {
    ...
    const dataSets = _.map(groupedByPlayerName, (grp, i) => {
      return {
        label: grp[0].PlayerName,
        data: grp.map(
          (gr: any) => gr.PlayerStats[this.statCategory][this.statToGraph]
        ),
        fill: false,
        yAxisID: 'y',
        tension: 0.4,
