...
export class InningService {
  ...
  nextInningForViewerClicked$: BehaviorSubject<Inning> = new BehaviorSubject(
    new Inning()
  );

  currentInningPlayers$: BehaviorSubject<InningPlayerView[]> =
    new BehaviorSubject([new InningPlayerView()]);
  currentInningPlayersInningsPitchedPerTeamLastThreeDays$: BehaviorSubject<
    any | null
  ...
  async createNewActiveInning(gameId: number, inningPlayers: InningPlayer[]) {
    ...
    this.createNewInningAndInningPlayers(
      gameId,
      inningPlayers,
      nextInningNumber
    );
  }
  async createNewInningAndInningPlayers(
    gameId: number,
    inningPlayers: InningPlayer[],
    nextInningNumber: number = 1
  ) {
  ...
  public addAnyNeededBenchPositions(data: any[]) {
    const newPostisions = this.addAnyBenchPositionsNeeded(
      data.length,
      this.positions
    );
  ...
  async insertInningPlayers(
    inningPlayersToAdd: InningPlayer[],
    inningId: number
  ) {
    await this.supabaseService.supabase
      .from('inningPlayers')
      .insert(inningPlayersToAdd);
    const currentActiveInningPlayers = await this.getCurrentActiveInningPlayers(
      inningId
    );
  ...
  public addAnyBenchPositionsNeeded(length: number, positions: any) {
    if (length > 10) {
      ...
      while (numberOfBenchToAdd > 0 && length != positions.length) {
        positions.push({
          label: 'B',
          value: 'B',
        });
  ...
  public async getPlayersInningPitchedLastThreeDays(
    teamId: number,
  ...
  public async getPreviousInningPlayers(currentInning: Inning) {
    const { data, error } = await this.supabaseService.supabase
      .from('innings')
      .select('id')
      .eq('gameId', currentInning.gameId)
      .eq('inningNumber', currentInning.inningNumber - 1);
    if (data && data[0]) {
      const inningNumberToGet: number = <number>data[0].id;
      return this.getCurrentActiveInningPlayers(inningNumberToGet);
    } else {
      return [];
    }
  }
